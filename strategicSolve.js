...
const solvePuzzleStrategically = (puzzle, goalState) => {
    ...
    puzzle.topRowInProgress = 0;
    puzzle.leftColumnInProgress = 0;
    puzzle.bottomRowInProgress = puzzle.rows - 1;
    puzzle.rightColumnInProgress = puzzle.cols - 1;
    ...
    while (!goalPuzzle.isEqualToPuzzle(puzzle)) {
        ...
        while ((puzzle.bottomRowInProgress + 1 - puzzle.topRowInProgress > 2) && (puzzle.bottomRowInProgress - puzzle.topRowInProgress + 1 >= puzzle.rightColumnInProgress + 1 - puzzle.leftColumnInProgress)) {
            ...
            if (Puzzle.isRowEqual(goalPuzzle, puzzle, puzzle.rowInProgress) && puzzle.rowInProgress !== goalPuzzle.blankRow) {

                // Increment up or down depending on which way we're solving
                if (puzzle.solvingRowTopDown) {
                    puzzle.topRowInProgress++;
                    puzzle.rowInProgress = puzzle.topRowInProgress;
                } else {
                    puzzle.bottomRowInProgress--;
                    puzzle.rowInProgress = puzzle.bottomRowInProgress;
                }
                ...
                while (!Puzzle.isRowEqual(goalPuzzle, puzzle, puzzle.rowInProgress)) {
                    ...
                    // Set strategy state on our puzzle object


                    // We are not on the last two tiles of the row, solve normally and increment across the row
                    if (targetValue !== goalState[puzzle.rowInProgress][puzzle.rightColumnInProgress - 1]) {
                        ...
                        */
                        const lastValue = goalState[puzzle.rowInProgress][puzzle.rightColumnInProgress];
                        if (puzzle.solvingRowTopDown) {
                            /*
        ...
        while ((puzzle.rightColumnInProgress + 1 - puzzle.leftColumnInProgress > 2) && (puzzle.rightColumnInProgress + 1 - puzzle.leftColumnInProgress > puzzle.bottomRowInProgress - puzzle.topRowInProgress + 1)) {
            ...
            if (Puzzle.isColEqual(goalPuzzle, puzzle, puzzle.colInProgress) && puzzle.colInProgress !== goalPuzzle.blankCol) {
                puzzle.colProgressRow = 0;

                // Increment up or down depending on which way we're solving
                if (puzzle.solvingColLeftRight) {
                    puzzle.leftColumnInProgress++;
                    puzzle.colInProgress = puzzle.leftColumnInProgress;
                } else {
                    puzzle.rightColumnInProgress--;
                    puzzle.colInProgress = puzzle.rightColumnInProgress;
                }
                ...
                let colIteration = 0;
                targetValue = goalState[puzzle.topRowInProgress][puzzle.colInProgress];
                while (!Puzzle.isColEqual(goalPuzzle, puzzle, puzzle.colInProgress)) {
                    ...
                    if (targetValue !== goalState[puzzle.bottomRowInProgress - 1][puzzle.colInProgress]) {
                        moveTile(puzzle, targetValue, goalMapping[targetValue].row, goalMapping[targetValue].col)
                        ...
                        /*
                          ...
                          These are special and need to be moved together
                        */
                        const lastValue = goalState[puzzle.bottomRowInProgress][puzzle.colInProgress]
        ...
        if ((puzzle.bottomRowInProgress + 1) - puzzle.topRowInProgress === 2 && (puzzle.rightColumnInProgress+1) - puzzle.leftColumnInProgress === 2) {
            let iterations = 0;
...
// puzzle { Puzzle: puzzle, int: rowInProgress, int: puzzle.colInProgress, bool: movingLastTwoTiles }
...
const moveTileLeft = (puzzle, tile) => {
    ...
    if (!puzzle.solvingRow && puzzle.solvingColLeftRight) {
        ...
        if (tile.col === puzzle.colInProgress + 1) {
            ...
            if (tile.row !== puzzle.bottomRowInProgress) {
...
const moveTileRight = (puzzle, tile) => {
    ...
    if (puzzle.solvingRow) {
        if (puzzle.solvingRowTopDown) {
            ...
            if (puzzle.blankRow === puzzle.rowInProgress && (puzzle.blankRow + 1 !== tile.row || puzzle.blankCol !== tile.col)) {
...
const moveTileUp = (puzzle, tile) => {
    ...
    if (puzzle.solvingRow && puzzle.solvingRowTopDown) {
        ...
        if (tile.row === puzzle.rowInProgress + 1) {
            ...
            // Not in last column
            if (tile.col !== puzzle.rightColumnInProgress) {
...
const moveTileDown = (puzzle, tile) => {
    ...
    if (!puzzle.solvingRow){
        ...
        if (puzzle.solvingColLeftRight) {
            ...
            if (puzzle.blankCol === puzzle.colInProgress && (puzzle.blankCol + 1 !== tile.col || puzzle.blankRow !== tile.row)) {
...
const moveBlankLeftOrRight = (puzzle) => {
    ...
    if (puzzle.blankCol === puzzle.rightColumnInProgress) {
        puzzle.slideLeft();
        puzzle.solutionMoves.push("LEFT");
    } else if (puzzle.blankCol === puzzle.leftColumnInProgress){
        puzzle.slideRight();
        puzzle.solutionMoves.push("RIGHT")
    } else {
...
const moveBlankUpOrDown = (puzzle) => {
    ...
    if (puzzle.blankRow === puzzle.topRowInProgress) {
        puzzle.slideDown();
        puzzle.solutionMoves.push("DOWN");
    } else if (puzzle.blankRow === puzzle.bottomRowInProgress){
        puzzle.slideUp();
        puzzle.solutionMoves.push("UP")
    } else {
        // Move blank in the default direction we're solving
